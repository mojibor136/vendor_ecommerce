    ...
    <div class="w-full h-full flex flex-col gap-4">
        ...
        <div class="flex flex-col bg-white shadow rounded md:p-6 p-4 md:gap-1 gap-3">
            ...
            <div class="flex justify-between items-center text-gray-600 text-sm">
                <p>
                    <a href="{{ route('admin.index') }}" class="text-blue-600 hover:underline">Home</a> / Gateway / Create
                </p>
                <a href="{{ route('payment-gateway.index') }}"
        ...
        <div class="w-full bg-white rounded md:px-6 px-3">
            <form action="{{ route('payment-gateway.store') }}" method="POST" enctype="multipart/form-data">
                @csrf
                ...
                <div class="mb-6 mt-6">
                    ...
                    <select name="gateway_name" id="gateway_name"
                        ...
                        <option value="BKash" {{ old('gateway_name') == 'BKash' ? 'selected' : '' }}>BKash</option>
                        <option value="Nagad" {{ old('gateway_name') == 'Nagad' ? 'selected' : '' }}>Nagad</option>
                    </select>
                    @error('gateway_name')
                ...
                <div class="mb-6 mt-6">
                    ...
                    <input type="text" name="app_key" id="app_key" placeholder="App Key"
                        class="w-full mt-2 p-2 border rounded border-gray-300 text-gray-700" value="{{ old('app_key') }}">
                    @error('app_key')
                        <div class="text-red-600 text-sm mt-1">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="mb-6 mt-6">
                    ...
                    <input type="text" name="username" id="username" placeholder="Username"
                        class="w-full mt-2 p-2 border rounded border-gray-300 text-gray-700" value="{{ old('username') }}">
                    @error('username')
                        <div class="text-red-600 text-sm mt-1">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="mb-6 mt-6">
                    ...
                    <input type="text" name="password" id="password" placeholder="Password"
                        class="w-full mt-2 p-2 border rounded border-gray-300 text-gray-700" value="{{ old('password') }}">
                    @error('password')
                        <div class="text-red-600 text-sm mt-1">{{ $message }}</div>
                    @enderror
                </div>
