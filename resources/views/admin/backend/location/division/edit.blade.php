    ...
    <div class="w-full h-full flex flex-col gap-4">
        ...
        <div class="flex flex-col bg-white shadow rounded md:p-6 p-4 md:gap-1 gap-3">
            <div class="flex justify-between items-center">
                <h2 class="text-2xl font-bold text-gray-800 mb-2">Division</h2>
            ...
            <div class="flex justify-between items-center text-gray-600 text-sm">
                <p>
                    <a href="{{ route('admin.index') }}" class="text-blue-600 hover:underline">Home</a> / Division / Edit
                </p>
                <a href="{{ route('division.index') }}"
        ...
        <div class="w-full bg-white rounded md:px-6 px-3">
            <form action="{{ route('division.update') }}" method="POST" enctype="multipart/form-data">
                ...
                <!-- Country Select -->
                <div class="mb-6">
                    <label for="country" class="block text-gray-700 font-medium">Select Country <span
                    ...
                    <select name="country_id" id="country"
                        ...
                        @foreach ($countries as $country)
                            <option value="{{ $country->id }}"
                                {{ $country->id == $division->country_id ? 'selected' : '' }}>
                                {{ $country->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <!-- Division Name -->
                <div class="mb-6 mt-6">
                    <label for="division_name" class="block text-gray-700 font-medium">Division Name<span
                            class="text-red-400"> *</span></label>
                    <input type="text" name="division_name" id="division_name" placeholder="Division Name"
                        class="w-full mt-2 p-2 border rounded border-gray-300 text-gray-700" value="{{ $division->name }}">
                    @error('division_name')
                        <div class="text-red-600 text-sm mt-1">{{ $message }}</div>
                    @enderror
                </div>
