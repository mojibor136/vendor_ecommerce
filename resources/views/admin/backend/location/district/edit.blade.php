    ...
    <div class="w-full h-full flex flex-col gap-4">
        ...
        <div class="flex flex-col bg-white shadow rounded md:p-6 p-4 md:gap-1 gap-3">
            ...
            <div class="flex justify-between items-center text-gray-600 text-sm">
                <p>
                    <a href="{{ route('admin.index') }}" class="text-blue-600 hover:underline">Home</a> / District / Edit
                </p>
                <a href="{{ route('district.index') }}"
            ...
            <form action="{{ route('district.update') }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="mb-6 mt-6">
                    ...
                    <input type="text" name="district_name" id="district_name" placeholder="District Name"
                        class="w-full mt-2 p-2 border rounded border-gray-300 text-gray-700" value="{{ $district->name }}">
                    @error('district_name')
                        <div class="text-red-600 text-sm mt-1">{{ $message }}</div>
                    @enderror
                </div>
