...
@section('title', 'Create Sub Slider')
@section('content')
    <div class="w-full h-full flex flex-col gap-4">
        <!-- Header Section -->
        <div class="flex flex-col bg-white shadow rounded md:p-6 p-4 md:gap-1 gap-3">
            <div class="flex justify-between items-center">
                <h2 class="text-2xl font-bold text-gray-800 mb-2">Create Sub Slider</h2>
            ...
            <div class="flex justify-between items-center text-gray-600 text-sm">
                <p>
                    <a href="{{ route('admin.index') }}" class="text-blue-600 hover:underline">Home</a> / Sub Sliders / Create
                </p>
                <a href="{{ route('slider.sub.index') }}"
        ...
        <div class="w-full bg-white rounded px-6 py-6 shadow">
            <form action="{{ route('slider.sub.store') }}" method="POST" enctype="multipart/form-data">
                @csrf
                ...
                <!-- Image Upload -->
                <div class="mb-4">
                    <label for="image" class="block text-gray-700 font-medium">Upload Image <span
                            class="text-red-500">*</span></label>
                    <input type="file" name="image"
                        class="w-full mt-2 p-2 border rounded border-gray-300 text-gray-700">
                    <small class="text-gray-500 text-xs">Only one image, exactly 600x300 pixels.</small>
                </div>
