    ...
    <div class="w-full h-full flex flex-col gap-4">
        ...
        <div class="flex flex-col bg-white shadow rounded md:p-6 p-4 md:gap-1 gap-3">
            ...
            <div class="flex justify-between items-center text-gray-600 text-sm">
                <p>
                    <a href="{{ route('admin.index') }}" class="text-blue-600 hover:underline">Home</a> / Company / Edit
                </p>
                <a href="{{ route('company.index') }}"
        ...
        <div class="w-full bg-white rounded md:px-6 px-3">
            <form action="{{ route('company.update') }}" method="POST" enctype="multipart/form-data">
                ...
                <!-- Editor Name Input -->
                <div class="mb-6 mt-6">
                    <label for="name" class="block text-gray-700 font-medium">Editor Name<span class="text-red-500">
                            *</span></label>
                    <input type="text" name="name" id="name" placeholder="Editor Name"
                        class="w-full mt-2 p-2 border rounded border-gray-300 text-gray-700" value="{{ $company->name }}">
                    @error('name')
                ...
                <div class="mb-6 mt-6">
                    ...
                    <input type="email" name="email" id="email" placeholder="Email"
                        class="w-full mt-2 p-2 border rounded border-gray-300 text-gray-700" value="{{ $company->email }}">
                    @error('email')
                        <div class="text-red-600 text-sm mt-1">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="mb-6 mt-6">
                    ...
                    <input type="text" name="fax" id="fax" placeholder="Fax"
                        class="w-full mt-2 p-2 border rounded border-gray-300 text-gray-700" value="{{ $company->fax }}">
                    @error('fax')
                        <div class="text-red-600 text-sm mt-1">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="mb-6 mt-6">
                    ...
                    <input type="text" name="phone" id="phone" placeholder="Phone"
                        class="w-full mt-2 p-2 border rounded border-gray-300 text-gray-700" value="{{ $company->phone }}">
                    @error('phone')
                        <div class="text-red-600 text-sm mt-1">{{ $message }}</div>
                    @enderror
                </div>
