...
@section('content')
    <div class="bg-white w-full h-full flex flex-col gap-3">
        ...
        <div class="px-4 pt-6 flex flex-col gap-1">
            <h1 class="text-2xl font-semibold text-gray-800">Deliverd Order</h1>
...
@push('scripts')
    ...
    <script>
        ...
        function fetchOrder(query = '', page = 1, startDate = '', endDate = '') {
            ...
            fetch(
                    `{{ route('delivered.order.api') }}?search=${query}&page=${page}&start_date=${startDate}&end_date=${endDate}`
                )
                        ...
                        const row = `
                            <tr class="border-b hover:bg-gray-100 transition-all">
                                ...
                                <td class="px-6 py-1 text-sm">
                                    <span title="${order.order,status}" class="px-2 py-1 rounded text-white text-xs font-medium ${statusColor}">
