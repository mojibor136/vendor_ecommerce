            ...
            <form action="{{ route('subscription.update') }}" method="POST">
                ...
                <div class="mb-4">
                    ...
                    <textarea name="description" id="description" rows="4"
                        class="w-full mt-2 p-2 border rounded border-gray-300 text-gray-700"
                        placeholder="Write subscription description...">{{ old('description', $subscription->description) }}</textarea>
                    @error('description')
                        <div class="text-red-600 text-sm mt-1">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="mb-4">
                    ...
                    <input type="number" name="product_limit" id="product_limit" placeholder="E.g. 5"
                        class="w-full mt-2 p-2 border rounded border-gray-300 text-gray-700"
                        value="{{ old('product_limit', $subscription->product_limit) }}">
                    @error('product_limit')
                        <div class="text-red-600 text-sm mt-1">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="my-4">
                    ...
                    <select name="duration_days" id="duration_days"
                        ...
                        <option value="15" {{ $subscription->duration_days == 15 ? 'selected' : '' }}>Free Trial
                        </option>
                        <option value="30" {{ $subscription->duration_days == 30 ? 'selected' : '' }}>Monthly</option>
                        <option value="360" {{ $subscription->duration_days == 360 ? 'selected' : '' }}>Yearly</option>
                    </select>
                    @error('duration_days')
                        <div class="text-red-600 text-sm mt-1">{{ $message }}</div>
                    @enderror
                ...
                <div class="mb-4">
                    ...
                    <input type="text" name="features" id="features" placeholder="Feature 1, Feature 2, Feature 3"
                        class="w-full mt-2 p-2 border rounded border-gray-300 text-gray-700"
                        value="{{ old('features', implode(', ', json_decode($subscription->features))) }}">
                    @error('features')
                        <div class="text-red-600 text-sm mt-1">{{ $message }}</div>
                    @enderror
                </div>
