...
@push('scripts')
    ...
    <script>
        ...
        function fetchCategories(query = '', page = 1) {
            fetch(`{{ route('seller.categories.api') }}?search=${query}&page=${page}`)
                    ...
                    data.data.forEach(category => {
                        const row = `
                            <tr class="border-b hover:bg-gray-100">
                                ...
                                <td class="px-6 py-1 text-gray-700 text-sm whitespace-nowrap">${category.seller_products_count}</td>
                                <td class="px-6 pt-4 text-center whitespace-nowrap">
